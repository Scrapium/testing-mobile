...
start_epoch = 1669766400

request_batch = 0

while start_epoch < 1669770000:
    start_epoch += 120
    request_complete = False
    request_batch += 1
    while not request_complete:
        data = twitter("$btc since:" + str(start_epoch + 0) + " until:" + str(start_epoch + 120), cursor)

        if(data):

            lines = json.dumps(data, indent=4)
            f = open("./samples/b" + str(request_batch) + "-r"+str(i) + '-request.json', 'w')
            for line in lines:
                f.write(line)
            f.close()
            i += 1
            ...
            for instruction in instructions:
                ...
                type_name = instruction["__typename"]

                if(type_name == "TimelineAddEntries"):

                    timeline_entries = instruction["entries"]
                    ...
                    for item in timeline_entries:
                        ...
                        if(entry_type == "TimelineTimelineModule"):
                            ...
                            timeline_typename = item["content"]["content"]["__typename"]

                            if(timeline_typename == "TimelineMessagePrompt"):
                                # tradingview chart
                                pass
                            elif(timeline_typename == "TimelinePrompt"):
                                # make twitter better
                                pass
                            elif(timeline_typename == "TimelineTweet"):
                                # get the tweet text:
                                tweet_text = item["content"]["content"]["tweet_results"]["result"]["legacy"]["full_text"]
                                print("> " + tweet_text)
                                f = open('tweets-btc.txt', 'a', encoding='utf-8')
                                f.write(tweet_text)
                                f.write("\n")
                                f.close()
                                tweet_count += 1
                    ...
                    pass
                elif(type_name == "TimelineReplaceEntry"):
                    if('entry' in instruction):
                        cursor_type = instruction["entry"]["content"]["cursor_type"]
                        if(cursor_type == "Bottom"):
                            cursor = instruction["entry"]["content"]["value"]

                    elif('entries' in instruction):
                        print("\n\n** For some reason found entries in instruction?! **\n\n")
