...
for oauth_consumer_key, oauth_consumer_secret, name in keys_and_secrets:
    ...
    print("Attempt token: ", name, oauth_consumer_key, oauth_consumer_secret)
    ...
    compiled_oauth_token = oauth_token # could be faulty
    ...
    features = json.loads(urllib.parse.unquote(query_params["features"][0]))
    variables = json.loads(urllib.parse.unquote(query_params["variables"][0]))
    ...
    # Assuming all needed variables are provided
    method = "GET"
    uri = "https://api.twitter.com/graphql/pAyVFRiMFMp9KCfffF7LHg"
    parameters = result

    # Assuming class methods functionality 
    def enc(value):
        return urllib.parse.quote(value, safe='')

    def pairize(parameters):
        return [f"{k}={v}" for k, v in parameters.items()]

    def nksort(parameters):
        return dict(sorted(parameters.items()))

    # Build body for signature
    body = '&'.join(map(enc2, [method, uri, '&'.join(pairize(nksort({k: enc2(v) for k, v in parameters.items()})))]))

    # Build key for signature
    key = '&'.join(map(enc2, [oauth_consumer_secret, oauth_token_secret]))

    # Build signature
    signature = enc(base64.b64encode(hmac.new(key.encode(), body.encode(), hashlib.sha1).digest()).decode())
    ...
    auth += 'oauth_timestamp="' + str(enc(oauth_timestamp)) + '", '
    auth += 'oauth_token="' + str(compiled_oauth_token) + '", ' 
    ...
    """
        ...
        oauth_nonce="41ECDF81-A0DE-470D-98F3-D7BD8DC0A77B",
