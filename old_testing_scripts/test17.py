...
for oauth_consumer_key, oauth_consumer_secret, name in keys_and_secrets:
    ...
    features_js = json.dumps(params_u["features"])
    variables_js = json.dumps(params_u["variables"])


    features_enc = urllib.parse.quote(features_js)
    variables_enc = urllib.parse.quote(variables_js)



    enc = auth.encode_params("https://api.twitter.com/graphql/pAyVFRiMFMp9KCfffF7LHg/HomeLatestTimeline", "GET", params_u)
    url_base = "GET&" + urllib.parse.quote("https://api.twitter.com/graphql/pAyVFRiMFMp9KCfffF7LHg/HomeLatestTimeline" , safe='') +  '?' +  enc 
    ...
    oauth_consumer_key = parsed_query['oauth_consumer_key'][0]
    oauth_nonce = parsed_query['oauth_nonce'][0]
    oauth_signature_method = parsed_query['oauth_signature_method'][0]
    oauth_timestamp = parsed_query['oauth_timestamp'][0]
    oauth_token = parsed_query['oauth_token'][0]
    oauth_version = parsed_query['oauth_version'][0]
    oauth_signature = parsed_query['oauth_signature'][0]
    ...
    headers = {
        'Authorization': 'OAuth oauth_signature="' + oauth_signature + '", oauth_nonce="' + oauth_nonce  + '", oauth_timestamp="' + oauth_timestamp + '", oauth_consumer_key="' + oauth_consumer_key + '", oauth_token="' + oauth_token + '", oauth_version="1.0", oauth_signature_method="HMAC-SHA1"',
    }
     
    try:
        url = parsed_url.geturl()
        response = requests.request("GET","https://api.twitter.com/graphql/pAyVFRiMFMp9KCfffF7LHg/HomeLatestTimeline",params={'features': features_enc, 'variables': variables_enc},  headers=headers)
        print(response.text)
