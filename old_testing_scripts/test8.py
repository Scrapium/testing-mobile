...
for oauth_consumer_key, oauth_consumer_secret, name in keys_and_secrets:
    try:
        ...
        compiled_oauth_token = guest_token + "-" + oauth_token # could be faulty
        ...
        testurl = "https://api.twitter.com/graphql/pAyVFRiMFMp9KCfffF7LHg/HomeLatestTimeline"

        data_to_enc = enc("GET&" + str(testurl) + "&oauth_consumer_key={}&oauth_nonce={}&oauth_signature_method=HMAC-SHA1&oauth_timestamp={}&oauth_token={}&oauth_version=1.0".format(
            oauth_consumer_key, oauth_nonce, oauth_timestamp, compiled_oauth_token)) # testurl could be faulty

        print(data_to_enc)

        signature = sign_request(oauth_consumer_secret, oauth_token_secret, data_to_enc.encode()).decode().replace("=", "%3D")
